...
## Output artifact
Working, verified code + accurate docs. `memory/progress.md` updated per task.
...
- Load `failures.md` (project + `~/.pi/agent/memory/`) at start; RE-READ before any retry.
  Match by failure KIND, apply the Prevention Rule, record `Applied:` on the task line.
